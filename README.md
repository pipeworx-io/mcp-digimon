...
`tools/list` at `https://gateway.pipeworx.io/digimon/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_digimon \
  -H 'Content-Type: application/json' \
  -d '{"name":"Agumon"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_digimon`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
