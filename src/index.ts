...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Digimon API');
}
...
async function getDigimon(args: Record<string, unknown>): Promise<unknown> {
  ...
  const res = await pwFetch(`${BASE}/digimon/${encodeURIComponent(name)}`, {
    headers: { Accept: 'application/json', 'User-Agent': UA },
  });

  if (res.status === 400 || res.status === 404) {
    return { error: `Digimon API: no Digimon matching "${name}". Call search_digimon to find valid names/ids.`, name };
  }
  if (!res.ok) {
